                ...
                <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 p-5">
                    ...
                    @if ($patient->medicalConditions->isNotEmpty())
                        <ul class="space-y-1.5 text-sm">
                            ...
                            <li class="flex items-center justify-between gap-2">
                                <span class="text-zinc-700 dark:text-zinc-300 truncate">Prueba del talón</span>
                                <span
                                    @class([
                                        'inline-block px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0',
                                        'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' => $patient->heel_prick_done === true,
                                        'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' => $patient->heel_prick_done === false,
                                        'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400' => $patient->heel_prick_done === null,
                                    ])
                                >
                                    {{ match ($patient->heel_prick_done) { true => 'Realizó', false => 'No realizó', null => '?',} }}
                                </span>
                            </li>
                        </ul>
                    @else
                    ...
                    <div class="mt-4 space-y-2">
                        @if ($patient->heel_prick_done === null)
                            <div
                                class="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 flex items-start gap-2"
                            >
                                <span class="text-amber-500 text-sm">⚠</span>
                                <p class="text-xs text-amber-700 dark:text-amber-300 mt-0.5">
                                    Falta registrar la
                                    <strong>Prueba del talón</strong>
                                    .
                                </p>
                            </div>
                        @endif
                        ...
                        @if ($missingVaccinesCount > 0)
                            <div
                                class="p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 flex items-start gap-2"
                            >
                                <span class="text-blue-500 text-sm">💉</span>
                                <p class="text-xs text-blue-700 dark:text-blue-300 mt-0.5">
                                    Tiene
                                    <strong>
                                        {{ $missingVaccinesCount }} vacuna{{ $missingVaccinesCount > 1 ? 's' : '' }}
                                    </strong>
                                    pendiente{{ $missingVaccinesCount > 1 ? 's' : '' }} de aplicar.
                                </p>
                            </div>
                        @endif

                        @if ($latestAppliedVaccine)
                            <div
                                class="p-3 rounded-lg bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800 flex items-start gap-2"
                            >
                                <span class="text-teal-500 text-sm">💉</span>
                                <p class="text-xs text-teal-700 dark:text-teal-300 mt-0.5">
                                    La última vacuna fue:
                                    <strong>{{ $latestAppliedVaccine->vaccine->name }}</strong>
                                    el {{ $latestAppliedVaccine->applied_at->format('d/m/Y') }}.
                                </p>
                            </div>
                        @endif
                    </div>
                        ...
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                            @foreach ($recentPrescriptions as $prescription)
                                @php
                                    ...
                                    $consultaReceta = $prescription->consultation;
                                @endphp

                                <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition">
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <span class="font-medium text-zinc-800 dark:text-zinc-200">
                                            {{ $consultaReceta?->consultation_date?->format('d/m/Y') ?? '—' }}
                                        </span>
                                        @if ($consultaReceta?->doctor)
                                            <span class="block text-xs text-zinc-400 dark:text-zinc-500">
                                                {{ $consultaReceta->doctor->full_name }}
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-right">
                                        <div class="inline-flex items-center gap-3">
                                            @if ($consultaReceta)
                                                ...
                                                <a
                                                    href="{{ route('consultas.show', $consultaReceta->id) }}#receta"
